...
export const formSubmission = defineType({
  ...
  fields: [
    ...
    defineField({
      name: 'availableDays',
      title: 'Megfelelő napok',
      description: 'Amelyik előválogató napokat a jelentkező megjelölte.',
      type: 'array',
      of: [{ type: 'string' }],
      options: { layout: 'tags' },
    }),
    defineField({ name: 'note', title: 'Megjegyzés a jelentkezőtől', type: 'text', rows: 3 }),
